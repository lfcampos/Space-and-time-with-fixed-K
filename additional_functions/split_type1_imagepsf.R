      ...
      if ((gam.mean21 < gam.mean22) & (sum(c(gam.mean11,gam.mean21) < emean.min)==0) & (sum(c(gam.mean12,gam.mean22) > emean.max)==0) & (min(gam.shape11,gam.shape12,gam.shape21,gam.shape22)>0)){
        ...
        if ((ewt1 > max(0,(ewt.com+u1-1)/u1)) & (ewt1 < min(1,ewt.com/u1))){
          ...
          if (sum(allocate_curr[,pick]==1)>0){
            ...
            p1 <- w1*psf(spatial[indcom,],mu1)*(ewt1*dgamma(energy[indcom],gam.shape11,gam.shape11/gam.mean11)+(1-ewt1)*dgamma(energy[indcom],gam.shape12,gam.shape12/gam.mean12))
            p2 <- w2*psf(spatial[indcom,],mu2)*(ewt2*dgamma(energy[indcom],gam.shape21,gam.shape21/gam.mean21)+(1-ewt2)*dgamma(energy[indcom],gam.shape22,gam.shape22/gam.mean22))
            probs.new <- p1/(p1+p2)
