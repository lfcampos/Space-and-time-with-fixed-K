mcmc.extended.full <- function(fix_runs,online_ordering,rjmcmc_run,w,allocate_curr,mu_curr,eparas,ewt,k_curr,mix_num){
  ...
  for (t2 in 1:fix_runs){
    ...
    probs[1:k_curr,] <- t(matrix(unlist(lapply(1:k_curr,function(i) w[i]*psf(spatial,mu_curr[,i])*(ewt[i]*dgamma(energy,eparas[i,3],eparas[i,3]/eparas[i,1])+(1-ewt[i])*dgamma(energy,eparas[i,4],eparas[i,4]/eparas[i,2])))),ncol=k_curr))
    probs[mix_num,] <- w[mix_num]*(1/img_area)*(1/max_back_energy)
    ...
    for (i in 1:k_curr){      
      ...
      if (count_vector[i]>0){
        ...
        logr <- sum(log(psf(spatial[index,],mu_prop[,i])))-sum(log(psf(spatial[index,],mu_curr[,i])))
        u <- runif(1,0,1)
        if(is.na(logr)==0){
          if (log(u) < logr){
            mu_curr[,i] <- mu_prop[,i]
