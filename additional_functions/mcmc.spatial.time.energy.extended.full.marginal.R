...
#   eparas: vector of energy parameters of length number of sources
#   ewt: vector of energy parameters of length number of sources
...
# rjmcmc_run = tt
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
mcmc.spatial.time.energy.extended.full <- function(fix_runs,online_ordering,rjmcmc_run,w,allocate_curr,mu_curr,eparas,ewt,k_curr,mix_num,bk,num_time_breaks, lambda){
  ...
  value <- list(c(k_curr,c(mu_curr),c(w)),allocate_curr, eparas, ewt,lambda)
  return(value)
}
