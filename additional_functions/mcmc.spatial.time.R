mcmc.spatial.time <- function(num_samples,thin,online_ordering,adapt_end,w,allocate_curr,mu_curr,k_curr,mix_num){
  ...
  for (t1 in 1:num_samples){
    ...
    for (t2 in 1:thin){
      ...
      # Modification: Fit light curve model
      #    1. column number mix_num (15 in the case of 14 sources) is background
      #    2. Number of blocks = 1 for sources with less than 10 photons
      #    3. Same model for 
      allocate_id = allocate_curr %*% 0:k_curr

      lambda = rates = vector('list', k_curr + 1)
      names(lambda) = names(rates) = 0:k_curr

      for(s in 0:k_curr){
        # subset to allocation
        t = arrival_time[allocate_id == s]
        # to subset by name instead of by index
        s = as.character(s)
        # count times given breakpoints for source s
        counts_s = multinomial.counts(t, breakpoints = bk[[s]])
        # generate dirichlet with corresponding counts (and prior)
        lambda[[s]] = rdirichlet(1, counts_s + lambda_prior)
        # get rates for all times accross sources for comparison
        rates[[s]] = get.rates(arrival_time, bk[[s]], lambda[[s]])
      }


      rates_curr = do.call('rbind', rates)
    ...
    paras_store[[t1]] <- list(c(k_curr,c(mu_curr),c(w)),allocate_curr,log_posterior_spatial(mu_curr,allocate_curr,w,k_curr), bk, lambda, rates_curr)
